"""A revisão preparada de `branches`, e o que a armadilha 53 cobra dela.

Eram duas. A `20260905_0058` — as cinco colunas de endereço mortas — **saiu
daqui em 05/09/2026** e está em `alembic/versions/`; quem cobra o resultado
dela agora é `tests/test_branches_perdeu_o_endereco_morto_db.py`, contra o
schema depois do `upgrade head`. O que sobrou aqui é a `20260905_0059`, que
move a tarifa de entrega para uma tabela 1:1 opcional e continua esperando
decisão. A proposta, com tamanho, está em `docs/modelo-de-dados.md`.

O que se cobra aqui é o mesmo das outras preparadas — o Alembic não a conhece,
ela ainda descreve o schema real, e ela **roda** —, com uma diferença que vale
a pena dizer: **esta move DADO**, e a `0057` não movia. Por isso os testes dela
põem linha na mesa antes de rodar a migração. Um `INSERT ... SELECT` sobre
tabela vazia é no-op, e provar só ele seria não provar nada — é a mesma lição
da revisão do cardápio por filial (armadilha 36).
...
def test_o_alembic_nao_conhece_esta_revisao():
    ...
    assert revisao.revision not in conhecidas, (
        f"{TARIFA.name} declara revision={revisao.revision!r} e o Alembic a "
