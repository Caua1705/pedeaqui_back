...
Uma só, em `sessao_service.py:56-79`:
...
|---|---|---|
| `_get_active_restaurant` | **sim** | `busca_service.py:54` e `rota.py:50` |
| `_build_restaurant_context` | **sim** (`@staticmethod`) | `rota.py:51` |
| `retrieval_service` (atributo) | não | `busca_service.py:56` |
| `_hydrate_products` | **sim** | `busca_service.py:61` |

**Três dos quatro pontos de acoplamento são privados.** O `ChatService` é
construído inteiro duas vezes por interação de voz: uma em `rota.py:50`
(rota da sessão) e outra em `busca_service.py:46` (cada tool call).
...
pagina.html:161   fetch POST /voice/search
  ...
  → busca_service.py:61   chat_service._hydrate_products()
...
|---|---|---|---|
| Função de busca | `retrieve_products` | idem | `chat_service.py:135` / `busca_service.py:56` |
...
| `top_k` | 5 | 5 | default em `retrieval_service.py:29`; **nenhum dos dois chamadores passa o valor** (`chat_service.py:135-138`, `busca_service.py:56-59`) |
...
para os dois. Mas a voz **descarta esse resultado**: `busca_service.py:60` lê
...
**Não existe em lugar nenhum.** Nem na tool (`sessao_service.py:64-78`), nem
...
1. **Assinatura da tool**, `sessao_service.py:64-78` — um campo novo em
...
cliente falou (`pagina.html:155` → `rota.py:63`). O modelo reformula a mesma
intenção de formas diferentes a cada turno, então a taxa de acerto do cache
na voz deve ser bem menor que na de texto. **Não medido** — é raciocínio sobre
o mecanismo, não medição.
...
`busca_service.py:46`), que constrói um `OpenAIEmbeddings` novo. Não conferi
...
|---|---|---|---|
| `restaurant_context` com nome e descrição | **sim** | **sim** | mesma função: `chat_service.py:183`, chamada em `:140` (texto) e `rota.py:51` (voz) |
| Preço da linha viva de `products`, fora do cache | **sim, por outro caminho** | **sim** | texto: `retrieval_service.py:73` → `:107`. Voz: descarta isso (`busca_service.py:60`) e usa o preço da hidratação (`busca_service.py:82`), que também é do banco |
| Regra "não citar produto que não veio da busca" — **no prompt** | **sim** | **sim** | `prompt_de_voz.py:31` / `system_prompt.py:16` |
...
| `selected_product_ids` + resgate de seleção vazia | **não** | **sim** | `chat_service.py:145` e `:149`. Na voz o modelo não devolve id nenhum — por desenho (`busca_service.py:51-52`) |
...
(`prompt_de_voz.py:38`) — e o que a ferramenta devolve está em outro formato
...
(`rota.py:74`). O que ele *pode* fazer é **falar** de um produto que não
existe — e aí não há código nenhum entre isso e o ouvido do cliente, porque a
saída é áudio.
...
(`pagina.html:96-107`, `POST https://api.openai.com/v1/realtime/calls`).

A única coisa que chega ao backend é a string `consulta` **gerada pelo
modelo** (`pagina.html:155` → `:162` → `rota.py:63`) — não é o que o cliente
falou, é a reformulação que o modelo escolheu.

As transcrições existem como eventos da Realtime e são tratadas em
`pagina.html:129-134`, mas só vão para o painel de log **no navegador**.
...
(`sessao_service.py:102`), e dela só o `expires_at` é registrado
(`sessao_service.py:136`). O que acontece depois — duração da conversa,
...
|---|---|---|
| Login | **não** | `rota.py:49` e `:63` dependem só de `get_db`; não há `get_current_customer` |
| Rate limit | **não** | nenhum `@limiter.limit` em `rota.py` (contraste: `src/api/chat.py:25` e `:35`) |
| Teto de duração da sessão | **não** | nada no corpo enviado em `sessao_service.py:90-99` |
...
|---|---|---|
| Renomear/remover `_get_active_restaurant` | `AttributeError` em runtime, nas **duas** rotas de voz | `busca_service.py:54`, `rota.py:50` |
| Mudar a assinatura ou o retorno de `_hydrate_products` | a tool call quebra; `resumo_para_o_modelo` lê `.name` e `.price` do que vier (`busca_service.py:82`) | `busca_service.py:61` |
| Tornar `_build_restaurant_context` não estático, ou mudar o texto que ele devolve | a emissão da credencial quebra ou passa instrução errada | `rota.py:51` |
| Mudar `ChatService.__init__` (exigir mais argumentos) | as duas rotas de voz | `busca_service.py:46`, `rota.py:50` |
| Renomear o atributo `retrieval_service` | a tool call | `busca_service.py:56` |
| Mudar o formato de `retrieve_products` (ex.: `id` deixar de ser chave do dict) | `busca_service.py:60` levanta `KeyError` | `busca_service.py:60` |
...
|---|---|---|
| Formato do preço mostrado ao modelo | `retrieval_service.py:107` (`format_money_br`) | `busca_service.py:82` (`:.2f`) — **já divergiram** |
| Regra "nunca invente produto" | `system_prompt.py:15-16` | `prompt_de_voz.py:31-32` |
...
1. **Formato do preço na voz** — trocar `:.2f` por `format_money_br` em
   `busca_service.py:82`. Uma linha, e é a única divergência que já produz
   comportamento diferente hoje.
2. **`restaurant_id` e digest da consulta no log da voz** —
   `busca_service.py:63` e `sessao_service.py:133`. Duas linhas, e sem isso
...
listados em `sessao_service.py:1-38`.
